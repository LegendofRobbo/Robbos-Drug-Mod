...
local rekt = false
if table.ToString(drugtab, false) == "{}" then chat.AddText( Color(0,205,50), fag:Nick().." is clean" ) return end
chat.AddText( Color(0,205,50), fag:Nick().." is under the influence of: " )
...
if rekt then chat.AddText( Color(255,0,0), "WARNING: ILLEGAL DRUGS DETECTED!" ) else chat.AddText( Color(0,205,50), "No illegal substances detected" ) end

end)

surface.CreateFont( "Aesthetic", {
	font = "Trebuchet MS",
	extended = false,
	size = 50,
	weight = 200,
...
--	draw.SimpleText( Drugmod_Buffs[k].Description, "PopuliHUD2", 5, ScrH() / 2 - ( (i * 30) - 30 ), Color(200,200,200,255), 0, 0 )
	...
	if k == "Vaporwave" then
		...
		local meme = math.abs(math.sin(CurTime() * 0.5)) / 1.75
		local meme2 = math.abs(math.cos(CurTime() * 0.9)) / 1.75
		local meme3 = math.abs(math.sin(CurTime() * 0.25)) / 1.75
		local meme4 = (math.abs(math.sin(CurTime() * 0.05)) / 2) + 0.5
		DrawColorModify( {
			[ "$pp_colour_addr" ] = meme3,
			[ "$pp_colour_addg" ] = meme,
			[ "$pp_colour_addb" ] = meme2,
			[ "$pp_colour_brightness" ] = 0,
			[ "$pp_colour_contrast" ] = meme4,
			[ "$pp_colour_colour" ] = 1,
			[ "$pp_colour_mulr" ] = 0,
			[ "$pp_colour_mulg" ] = 0,
			[ "$pp_colour_mulb" ] = 0,
...
hook.Add("HUDPaint", "DrawDrugs", function()
	...
	if !ent.DrugLegal then draw.SimpleText("[ILLEGAL]","Drugz_Font1",p.x,(p.y + offy) - 6 , Color(255,0,0),1, 1) end
...
--	if not smoothing then smoothing = fov end
	...
	view.origin = pos
	view.angles = ang
	view.fov = fov + (-SmoothVertical * 2) * dmul

	return GAMEMODE:CalcView( ply, pos, ang, fov )
end

hook.Add( "CalcView", "DM_Drunkview", Drunkview )



local muzak = -1
function Drugmod_VWMusic( b )
	if b then
		muzak = CreateSound( LocalPlayer(), "memes/shitmemes.mp3" )
		muzak:Play()
	else
		if !isnumber(muzak) then muzak:Stop() end
	end
